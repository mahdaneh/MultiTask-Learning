...
class MTL_pairwise(object):
    def __init__(self, ft_extrctor_prp, hypoth_prp, discrm_prp,  **kwargs):
        ...
        self.num_tr_smpl = kwargs['tr_smpl']
        self.num_test_smpl = kwargs['test_smpl']
    ...
    def fit(self):
        ...
        train_loader, test_loader, validation_loader = db.data_loading(self.img_size, self.num_tr_smpl, self.num_test_smpl,
                                                                       self.tsklist)
        ...
        for epoch in range(self.epochs):
            ...
            whole_loss  =  self._epoch_fit(train_loader, epoch)
            Total_loss.append(whole_loss)
            tasks_trAcc = self.epoch_eval(train_loader, epoch, 'train')
            tasks_valAcc = self.epoch_eval(validation_loader, epoch, 'validation')
            tasks_teAcc = self.epoch_eval(test_loader, epoch, 'test')
    ...
    def _epoch_fit(self, data_loader, epoch):
    ...
    def _epoch_eval(self, data_loader, epoch, phase='test'):
    ...
    def gradient_penalty(self, data_t, data_k, t, k):
        ...
        # computing gradient w.r.t interpolated sample
